...
ROLE_GROUP_ID=`curl -s -S -u $CM_USER:$CM_PASS $INSECURE "$API_URL/clusters/$CLUSTER/services/$SERVICE/roleConfigGroups" | grep name | grep '\-OOZIE_SERVER\-' | sed -e 's/.*"\(.*OOZIE_SERVER.*\)".*/\1/g'`
