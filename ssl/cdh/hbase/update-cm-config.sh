...
ROLE_GROUP_ID=`curl -s -S -u $CM_USER:$CM_PASS $INSECURE "$API_URL/clusters/$CLUSTER/services/$SERVICE/roleConfigGroups" | grep name | grep '\-HBASERESTSERVER\-' | sed -e 's/.*"\(.*HBASERESTSERVER.*\)".*/\1/g'`
...
ROLE_GROUP_ID=`curl -s -S -u $CM_USER:$CM_PASS $INSECURE "$API_URL/clusters/$CLUSTER/services/$SERVICE/roleConfigGroups" | grep name | grep '\-HBASETHRIFTSERVER\-' | sed -e 's/.*"\(.*HBASETHRIFTSERVER.*\)".*/\1/g'`
