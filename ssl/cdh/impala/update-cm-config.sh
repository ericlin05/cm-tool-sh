...
ROLE_GROUP_ID=`curl -s -u $CM_USER:$CM_PASS $INSECURE "$API_URL/clusters/$CLUSTER/services/$SERVICE/roleConfigGroups" | grep name | grep '\-IMPALAD\-' | sed -e 's/.*"\(.*IMPALAD.*\)".*/\1/g'`
...
ROLE_GROUP_ID=`curl -s -u $CM_USER:$CM_PASS $INSECURE "$API_URL/clusters/$CLUSTER/services/$SERVICE/roleConfigGroups" | grep name | grep '\-CATALOGSERVER\-' | sed -e 's/.*"\(.*CATALOGSERVER.*\)".*/\1/g'`
...
ROLE_GROUP_ID=`curl -s -u $CM_USER:$CM_PASS $INSECURE "$API_URL/clusters/$CLUSTER/services/$SERVICE/roleConfigGroups" | grep name | grep '\-STATESTORE\-' | sed -e 's/.*"\(.*STATESTORE.*\)".*/\1/g'`
