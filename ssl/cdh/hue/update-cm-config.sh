...
ROLE_GROUP_ID=`curl -s -u $CM_USER:$CM_PASS $INSECURE "$API_URL/clusters/$CLUSTER/services/$SERVICE/roleConfigGroups" | grep name | grep '\-HUE_SERVER\-' | sed -e 's/.*"\(.*HUE_SERVER.*\)".*/\1/g'`
...
ROLE_GROUP_ID=`curl -s -u $CM_USER:$CM_PASS $INSECURE "$API_URL/clusters/$CLUSTER/services/$SERVICE/roleConfigGroups" | grep name | grep '\-HUE_LOAD_BALANCER\-' | sed -e 's/.*"\(.*HUE_LOAD_BALANCER.*\)".*/\1/g'`
