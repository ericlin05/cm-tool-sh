...
ROLE_GROUP_ID=`curl -s -S -u $CM_USER:$CM_PASS $INSECURE "$API_URL/clusters/$CLUSTER/services/$SERVICE/roleConfigGroups" | grep name | grep '\-HIVESERVER2\-' | sed -e 's/.*"\(.*HIVESERVER2.*\)".*/\1/g'`
